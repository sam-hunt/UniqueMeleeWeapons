...
# the 1.6/Mods/Royalty compat load root (the unique Axe/Warhammer and their
# Royalty-tech traits/colours), whose defs would drop out of a dump made
# without it, turning its shipped translations illegal.
...
def def_is_active(elem, active_packages):
    ...
    # absent from the sidecar and outside the l10n surface (e.g. this mod's own
    # Royalty-gated UMW_Warhammer_Unique/UMW_Axe_Unique ThingDefs).
...
def check_language(lang_dirs, english_keyed, sidecar, def_roots, report):
    ...
    # root is not a language of its own: UMW_ZeusHeaded/UMW_PlasmaCored/
    # UMW_Monomolecular's injections can only live under
    # 1.6/Mods/Royalty/Languages/<Lang>/ (MayRequire is ignored on DefInjected,
    # so the LoadFolders gate has to be the folder), yet the sidecar marks them
    # required — checking that root standalone would demand the whole Keyed set
    # and every other def's injections from it.
