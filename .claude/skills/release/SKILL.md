...
- Boots the freshly deployed build once on a pinned list of UMW plus its
  family siblings (UWU, PWU — graphical boot, ~1-2 min, auto-quits), then
  classifies every Player.log error by origin and fails on anything
  attributed to UMW or a family seam. This is the only automated coverage
  the cross-mod `TraitEffectLinesExtension` reflection contract with UWU
  gets (see CLAUDE.md's Debugging section).
